...
async function getRates() {
  ...
  try {const BASE = window.location.origin;
const API = {
  ml: `${BASE}/api/mercadolibre`,
  ebay: `${BASE}/api/ebay`,
  cex: `${BASE}/api/cex`,
...
// ... (Tus variables BASE y API iniciales se mantienen igual)
...
async function buscar() {
  ...
  // DEFINICIÓN DE RUN (Línea 55 aproximadamente)
  const run = async (fn, resultId, color, currency = 'MXN') => {
    try {
      const data = await fn();
      let rawItems = data.results || data || [];
      
    // Dentro de tu script.js en la función buscar() -> run()
let items = rawItems.filter(item => {
  const p = Number(item.price);
  const t = item.title.toLowerCase();
  
  // 1. Eliminamos precios de $0 o errores de carga
  if (p <= 0) return false;

  // 2. Filtro agresivo de "basura"
  const palabrasProhibidas = [
    'arete', 'stainless steel', 'acero inoxidable', // Elimina la joyería
    'novel', 'libro', 'facts', 'biografía',          // Elimina libros
    'playera', 'camisa', 't-shirt', 'póster'         // Elimina ropa/merch
  ];
  ...
  return !esBasura;
      ...
      // Enlaces de respaldo para el botón de error
      const links = {
  ...
  }; // AQUÍ SE CIERRA LA FUNCIÓN RUN CORRECTAMENTE
  ...
  // CheapShark: Ahora redirige correctamente a la tienda
